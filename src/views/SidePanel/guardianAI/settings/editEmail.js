...
import { guardianMessageListPrevention, guardianMessageListRecovery, guardianMessageUpdate } from '../store'

const EditEmail = ({ modalOpen, handleCloseModal, emailData, messageCampaign }) => {
    ...
    const changeHandler = (e, key) => {
        ...
        if(key === "subject"){
            setCountSubject(e.target.value.length)
        }
    }
    ...
    const onSubmit = (data) => {
        ...
        const obj = {
            ...
            messageSubject: data.emailSubject,
            messageActiveStatus: emailData.messageActiveStatus,
            messageTitle: data.emailSubject,
            id: emailData?.id
        }
    ...
    useEffect(() => {
        setValue("emailSubject", emailData?.subject)
        setValue("emailBody", emailData?.body)
        setCountSubject(emailData?.subject?.length)
        setCountBody(emailData?.body?.length)
    }, [emailData])

    return (
        <div>
            <Modal isOpen={modalOpen} toggle={handleCloseModal} className="custom-modal modal-dialog-centered">
                <ModalHeader toggle={handleCloseModal}>Edit Email</ModalHeader>
                <ModalBody>
                    <Form
                                ...
                                <Controller
                                    ...
                                    render={({ field : {onChange, value} }) => (
                                        ...
                                        <div className="floating email-edit-floating position-relative">
                                            ...
                                            <div className='w-100'>
                                                <input id="input__subject" className="floating__input form-control" value={value} name="subject" type="text" placeholder='Subject' maxlength={200} onChange={(e) => 
                                                   {
                                                    onChange(e.target.value)
                                                    changeHandler(e, "subject")
                                                   } 
                                                    } />
                                                <label for="input__subject" className="floating__label" data-content="Subject">
                                                </label>
                                                <p className='max-text'>{countSubject}/200</p>
                                            </div>
                                        </div>
                                    )}
                                />
                                ...
                                <Controller
                                    ...
                                    render={({ field : {onChange, value}}) => (
                                        <div className="floating email-edit-floating position-relative">
                                            ...
                                            <div className='w-100'>
                                                <textarea id="input__emailbody" className="floating__input form-control" value={value} name="emailBody" placeholder='EmailAccount Number' maxlength={2000} onChange={(e) => 
                                                   {
                                                    onChange(e.target.value)
                                                ...
                                                <p className='max-text'>{countBody}/2000</p>
                                            </div>
                                        </div>
